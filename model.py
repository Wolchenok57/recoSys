...
# -*- coding: utf-8 -*-
"""
model.py — RAGEncoder с RoPE, GQA, SwiGLU, RMSNorm.
Добавлена поддержка:
- заморозки эмбеддингов (freeze_embeddings)
- pooling: 'cls' (BOS) или 'mean'
- загрузка чужих эмбеддингов с возможностью разморозки
"""
...
class RAGEncoder(nn.Module):
    def __init__(self,
        ...
        """
        Args:
            freeze_embeddings: заморозить слой tok_emb.weight (по умолч. True для совместимости)
            pooling: 'cls' – использовать первый токен (BOS), 'mean' – среднее по всем токенам с учётом маски
        """
        super().__init__()
        self.dim = dim
        ...
        self.tok_emb.weight.requires_grad_(not freeze_embeddings)

    def load_embeddings(self, path: str, freeze: Optional[bool] = None):
        """
        Загружает внешние эмбеддинги в tok_emb.weight.
        Если freeze не указан, сохраняется текущее состояние requires_grad.
        """
        ...
        # Распаковка распространенных форматов чекпоинтов
        state_dict = ckpt
        ...
        if isinstance(state_dict, dict):
            for k in target_keys:
                if k in state_dict:
                    ...
                    break
            # Фоллбэк: поиск по ожидаемой форме
        ...
        frozen_state = not self.tok_emb.weight.requires_grad
        print(f"[✓] Embeddings loaded from {path} | Shape: {self.tok_emb.weight.shape} | Frozen: {frozen_state}")
    ...
    def forward(self, tokens: torch.Tensor, attention_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if self.pooling == 'cls':
            # Берём первый токен (предполагается BOS_TOKEN_ID)
            hidden = x[:, 0, :]
        else:  # 'mean'
            if attention_mask is None:
                # Если маски нет, считаем все токены
                mask = torch.ones_like(tokens, dtype=torch.float32)
            else:
                mask = attention_mask.float()
            mask = mask.unsqueeze(-1)          # [B, L, 1]
            hidden = (x * mask).sum(dim=1) / mask.sum(dim=1).clamp_min(1e-9)
...
if __name__ == '__main__':
    ...
    # Пример создания модели с размороженными эмбеддингами и mean pooling
    model = RAGEncoder(freeze_embeddings=False, pooling='mean')
    print(f"📦 Размер словаря: {VOCAB_SIZE}")
    print(f"📐 Конфигурация: dim={MODEL_DIM}, layers={N_LAYERS}, heads={N_HEADS}/{N_KV_HEADS}")
    print(f"🔧 Pooling: {model.pooling}, Embeddings trainable: {model.tok_emb.weight.requires_grad}")
    ...
    if os.path.exists(ckpt_path):
        try:
            model.load_embeddings(ckpt_path, freeze=False)   # переопределяем заморозку
            trainable_after = model.get_num_params(trainable_only=True)
            print(f"📊 Обучаемых параметров (после загрузки эмбеддингов): {trainable_after / 1e6:.2f}M")
    ...
    dummy_tokens = torch.tensor([[BOS_TOKEN_ID, 15, 22, 10, 4, 0, 0, 0]], device=device)
